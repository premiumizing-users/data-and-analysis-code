...
OR b.device_gpu IN ('Adreno (TM) 540','Mali-T624','Adreno (TM) 530','Adreno (TM) 418','Mali-T628')
OR b.device_ram > 4000)
THEN 'Android tier 1'
WHEN b.device_os='android'
...
OR b.device_gpu in ('Mali-G72','Adreno (TM) 510','Adreno (TM) 330','Adreno (TM) 420','Mali-G71')
OR b.device_ram > 3000)
THEN 'Android tier 2'
WHEN b.device_os='android'
AND (b.device_model in ('SM-G950F','VTR-L09','SM-G920F','ONEPLUS A3003','SM-G900F')
OR b.device_gpu IN ('Adreno (TM) 430','Mali-T880','Adreno (TM) 305','Mali-T760')
OR b.device_ram > 2500)
THEN 'Android tier 3'
WHEN b.device_os='android'
AND b.device_ram > 1500
THEN 'Android tier 4'
...
WHERE
CAST(ujd AS DATE) >= DATE('2019-02-27')
...
MAX(CAST(CASE WHEN CAST(join_date AS DATE) = CAST(date AS DATE) AND device_ram IS NOT NULL THEN device_ram END AS INT64)) device_ram,
MAX(CASE WHEN CAST(join_date AS DATE) = CAST(date AS DATE) THEN device_gpu ELSE NULL END) device_gpu,
MAX(CASE WHEN CAST(join_date AS DATE) = CAST(date AS DATE) THEN device_cpu ELSE NULL END) device_cpu,
...
WHERE
CAST(join_date AS DATE) >= DATE('2019-02-27')
...
AND CAST(ujd AS DATE) >= DATE('2019-02-27')
AND CAST(ujd AS DATE) <= DATE_ADD(CURRENT_DATE(), INTERVAL -8 DAY)
GROUP BY
1,2)
GROUP BY
...
AND a.join_date >= DATE('2019-02-27')
AND a.join_date <= DATE('2019-03-21')
